...
/* =========================
   Schnellauswahl
   ========================= */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['assign_box'])) {
    if (($_POST['csrf_token'] ?? '') !== $_SESSION['csrf_token']) die("CSRF mismatch");

    $itemId = (int)$_POST['item_id'];
    $boxId  = (int)$_POST['box_id']; // 0 = Keine Box

    $db->prepare("UPDATE tackle SET box_id = ? WHERE id = ?")
       ->execute([$boxId, $itemId]);

    header("Location: index.php?id=".$itemId);
    exit;
}
...
<div class="action-btns" style="grid-template-columns: 1fr 1fr 1fr;">

    <!-- ✏️ Bearbeiten -->
    <a href="?id=<?= $item['id'] ?>&edit=1"
    ...
    <form method="POST" style="margin:0;">
        <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
        <input type="hidden" name="assign_box" value="1">
        ...
        <select name="box_id"
                ...
                style="
                    ...
                    cursor:pointer;
                ">
            <option value="0"><?= $t['no_box'] ?></option>

            <?php foreach($boxes as $bx): ?>
                <option value="<?= $bx['id'] ?>"
                    <?= $item['box_id']==$bx['id'] ? 'selected' : '' ?>>
