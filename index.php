...
$texts = [
    'de' => [
        ...
        'cats' => ["Metallköder", "Hardbaits", "Gummiköder", "Angelruten", "Rollen", "Haken", "Zubehör"],
    ...
    'en' => [
        ...
        'cats' => ["Metal Baits", "Hardbaits", "Softbaits", "Rods", "Reels", "Hooks", "Accessories"],
...
if (!file_exists($configFile)) {
    ...
    echo '<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width, initial-scale=1.0"><style>body{background:#0f172a;color:#fff;font-family:sans-serif;display:flex;justify-content:center;align-items:center;height:100vh;margin:0;} .box{background:#1e293b;padding:30px;border-radius:12px;width:90%;max-width:320px;box-shadow:0 10px 25px rgba(0,0,0,0.3);position:relative;} input{width:100%;padding:12px;margin:10px 0;background:#0f172a;border:1px solid #334155;color:#fff;border-radius:8px;box-sizing:border-box;font-size:16px;} button{width:100%;padding:12px;background:#38bdf8;border:none;border-radius:8px;font-weight:bold;cursor:pointer;margin-top:10px;} .lang-switch{position:absolute;top:10px;right:10px;text-decoration:none;color:#38bdf8;font-size:0.8rem;font-weight:bold;}</style></head>';
    echo '<body><div class="box"><a href="?lang='.($lang=='de'?'en':'de').'" class="lang-switch">🌐 '.$t['lang_btn'].'</a><h2>'.$t['setup_title'].'</h2><p style="color:#94a3b8;font-size:0.9rem;">'.$t['setup_desc'].'</p><form method="POST"><input type="text" name="set_user" placeholder="'.$t['user'].'" required><input type="password" name="set_pass" placeholder="'.$t['pass'].'" required><button type="submit">'.$t['create'].'</button></form></div></body></html>';
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['login_user'], $_POST['login_pass'])) {
    ...
    if ($_SESSION['login_attempts'] > 15) die("Too many attempts.");
...
if (!isset($_SESSION['logged_in']) || $_SESSION['logged_in'] !== true) {
    ...
    echo '<body><div class="box"><a href="?lang='.($lang=='de'?'en':'de').'" class="lang-switch">🌐 '.$t['lang_btn'].'</a><h2>🎣 '.$t['title'].' Login</h2>';
...
if (isset($_GET['backup_export'])) {
    ...
    $zipName = 'tacklebox-backup.zip';
    $zip = new ZipArchive();

    if ($zip->open($zipName, ZipArchive::CREATE | ZipArchive::OVERWRITE) !== true) {
        abort_app("Failed to create backup archive.");
    }
    ...
    if (!file_exists($dbFile)) {
        abort_app("Database file not found for backup.");
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['restore_file'])) {
    ...
    if (($_POST['csrf_token'] ?? '') !== $_SESSION['csrf_token']) {
        abort_app("CSRF validation failed.");
    }

    $zip = new ZipArchive();
    if ($zip->open($_FILES['restore_file']['tmp_name']) !== true) {
        abort_app("Invalid backup ZIP file.");
    }

    $tmp = sys_get_temp_dir() . '/tacklebox_import_' . uniqid();
    mkdir($tmp, 0755, true);
    $zip->extractTo($tmp);
    $zip->close();

    if (!file_exists($tmp.'/meta.json') || !file_exists($tmp.'/database.sqlite')) {
        abort_app("Backup is not compatible.");
    }

    copy($tmp.'/database.sqlite', $dbFile);
    chown($dbFile, 'www-data');
    chmod($dbFile, 0660);

    if (is_dir($tmp.'/uploads')) {
        if (!is_dir('uploads')) mkdir('uploads', 0755, true);
        foreach (glob($tmp.'/uploads/*') as $f) {
            copy($f, 'uploads/'.basename($f));
        }
    }
    ...
    <?php elseif ($item): ?>
        <div style="text-align:center;">
            ...
            <div class="mini-stats">
                <div class="mini-card"><label><?= $t['color'] ?></label><b><?= htmlspecialchars($item['farbe'] ?: '-') ?></b></div>
